...
/*   Updated: 2023/10/30 16:09:42 by pgorner          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::invite(std::vector<std::string> tokens, int cc)
{
	...
	if (_clients[cc].admin == false && !client_oper(cc, find_chan(tokens[1]))){_clients[cc].send_to_user += irc::cEM(irc::ERR_NOPRIVILEGES());}
